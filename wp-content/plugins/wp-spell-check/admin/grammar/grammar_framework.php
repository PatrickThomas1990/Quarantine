...
function wpgcx_check_pages($rng_seed = 0, $is_running = false, $log_debug = true) {
...
function wpgcx_check_posts($rng_seed = 0, $is_running = false, $log_debug = true) {
	$scanner = new wpscx_grammar_scanner;
...
function wpsc_create_meta_box ( $post ) {
	...
	if ($_GET['wpsc-scan-page'] == 1) {
						...
						foreach($wpsc_data as $row) {
							?>
								<tr>
									...
									<td style="padding: 5px 10px;"><?php echo $row['page_name']; ?></td>
								</tr>
							<?php
						}
					?>
		...
		$wpsc_data = $wpdb->get_results("SELECT * FROM $table WHERE ignore_word is false AND page_id=$id");
...
function wpgcx_create_meta_box( $post ) {
	if (isset($_GET['wpgc-scan-page-grammar'])) { if ($_GET['wpgc-scan-page'] == "Gramme Check") wpgcx_scan_individual($post->ID); }
}
...
function wpgcx_highlight_errors( $post ) {
	...
	//print_r(json_encode($spelling_highlight));
		...
		<script type="text/javascript">
			...
			jQuery(document).ready(function($) {
				...
				jQuery('.wpgc-scan-page-grammar').click(function() {
					window.location.href = "<?php echo "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]&wpgc-scan-page=Gramme Check"; ?>";
				});
				...
				window.setTimeout(function(){ 
					...
					iframe.find('.hiddenSpellError').click(function(e) {
						...
						if (jQuery(this).parents('.excerpt').length) {
                                                    jQuery(".wpgc-dialog").css( "top", e.pageY + 720);
                                                } else {
                                                    jQuery(".wpgc-dialog").css( "top", e.pageY + 200);
                                                }
						jQuery(".wpgc-dialog").css( "left", e.pageX);
						jQuery(".wpgc-dialog").css( "display", "block");
						jQuery('.wpgc-dialog-close').click(function(e) {
							//var scroll_save = jQuery(window).scrollTop();							
							//word_to_edit.css("background","inherit");
                                                        word_to_edit.closest('.hiddenSpellError').contents().unwrap();
                                                        //console.log(word_to_edit);
                                                        //jQuery(".wp-editor-area#content").contents().find(".hiddenSpellError").contents().unwrap();
...
function wpgcx_publish_box() {
	...
	if ($post_id != "") {
		...
		if (isset($_GET['wpgc-scan-page-grammar'])) { if ($_GET['wpgc-scan-page-grammar'] == "Grammer Check") wpgcx_scan_individual($post_id); }
		...
		$spell_score = $wpdb->get_results("SELECT * FROM $spell_table WHERE page_id=$post_id AND ignore_word=0 AND (page_type='Page Content' or page_type='Post Content')");
                ...
                if (isset($_GET['wpgc-scan-page'])) { if ($_GET['wpgc-scan-page'] == "Spell Check") { $spellcheck_scanner = new wpscx_spellcheck_scanner (); $spell_score = $spellcheck_scanner->scan_single($post_id); } }
		...
		?>
			...
			<?php if($_GET['wpgc-scan-page'] == "Gramme Check") { ?>
...
function wpgcx_finish_scan() {
	...
	if ($ent_included) {
		...
		wpscx_print_debug_end("6.8.4 Grammar Check Pro",$total_time);
	} else {
		$end = round(microtime(true),5);
		$total_time = round($end - $start, 5);
		wpscx_print_debug_end("6.8.4 Grammar Check Base",$total_time);
	}
}
